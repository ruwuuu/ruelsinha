...
// node_modules/@embedpdf/snippet/dist/worker-engine-m0rlHn8R.js
var i = "RemoteExecutor";
var s = "Worker";
var d = class n {
  constructor(e, d2) {
    this.worker = e, this.pendingRequests = /* @__PURE__ */ new Map(), this.requestCounter = 0, this.handleMessage = (n2) => {
      const e2 = n2.data;
      if ("ready" === e2.type) return this.logger.debug(i, s, "Worker is ready"), void this.readyTask.resolve(true);
      const r = this.pendingRequests.get(e2.id);
      if (r) switch (e2.type) {
        case "result":
          this.logger.debug(i, s, `Received result for ${e2.id}`), r.resolve(e2.data), this.pendingRequests.delete(e2.id);
          break;
        case "error":
          this.logger.debug(i, s, `Received error for ${e2.id}:`, e2.error), e2.error ? r.fail(e2.error) : r.reject({ code: pn.Unknown, message: "Unknown error" }), this.pendingRequests.delete(e2.id);
          break;
        case "progress":
          this.logger.debug(i, s, `Received progress for ${e2.id}`), r.progress(e2.progress);
      }
      else this.logger.warn(i, s, `Received response for unknown request: ${e2.id}`);
    }, this.logger = d2.logger ?? new ft(), this.worker.addEventListener("message", this.handleMessage), this.readyTask = new It(), this.pendingRequests.set(n.READY_TASK_ID, this.readyTask), this.worker.postMessage({ id: n.READY_TASK_ID, type: "wasmInit", wasmUrl: d2.wasmUrl, logger: d2.logger ? Wn(d2.logger) : void 0, fontFallback: d2.fontFallback }), this.logger.debug(i, s, "RemoteExecutor created");
  }
  generateId() {
    return `req-${Date.now()}-${this.requestCounter++}`;
  }
  send(n2, e) {
    const r = this.generateId(), a = new It(), d2 = { id: r, type: "execute", method: n2, args: e };
    return this.readyTask.wait(() => {
      this.pendingRequests.set(r, a), this.logger.debug(i, s, `Sending ${n2} request:`, r), this.worker.postMessage(d2);
    }, (e2) => {
      this.logger.error(i, s, `Worker init failed, rejecting ${n2}:`, e2), a.reject({ code: pn.Initialization, message: "Worker initialization failed" });
    }), a;
  }
  destroy() {
    this.worker.removeEventListener("message", this.handleMessage), this.pendingRequests.forEach((e, t) => {
      t !== n.READY_TASK_ID && (e.abort("Worker destroyed"), this.logger.debug(i, s, `Aborted pending request: ${t}`));
    }), this.pendingRequests.clear(), this.worker.terminate(), this.logger.debug(i, s, "RemoteExecutor destroyed");
  }
  openDocumentBuffer(n2, e) {
    return this.send("openDocumentBuffer", [n2, e]);
  }
  getMetadata(n2) {
    return this.send("getMetadata", [n2]);
  }
  setMetadata(n2, e) {
    return this.send("setMetadata", [n2, e]);
  }
  getDocPermissions(n2) {
    return this.send("getDocPermissions", [n2]);
  }
  getDocUserPermissions(n2) {
    return this.send("getDocUserPermissions", [n2]);
  }
  getSignatures(n2) {
    return this.send("getSignatures", [n2]);
  }
  getBookmarks(n2) {
    return this.send("getBookmarks", [n2]);
  }
  setBookmarks(n2, e) {
    return this.send("setBookmarks", [n2, e]);
  }
  deleteBookmarks(n2) {
    return this.send("deleteBookmarks", [n2]);
  }
  renderPageRaw(n2, e, t) {
    return this.send("renderPageRaw", [n2, e, t]);
  }
  renderPageRect(n2, e, t, r) {
    return this.send("renderPageRect", [n2, e, t, r]);
  }
  renderThumbnailRaw(n2, e, t) {
    return this.send("renderThumbnailRaw", [n2, e, t]);
  }
  renderPageAnnotationRaw(n2, e, t, r) {
    return this.send("renderPageAnnotationRaw", [n2, e, t, r]);
  }
  renderPageAnnotationsRaw(n2, e, t) {
    return this.send("renderPageAnnotationsRaw", [n2, e, t]);
  }
  getPageAnnotationsRaw(n2, e) {
    return this.send("getPageAnnotationsRaw", [n2, e]);
  }
  getPageAnnotations(n2, e) {
    return this.send("getPageAnnotations", [n2, e]);
  }
  createPageAnnotation(n2, e, t, r) {
    return this.send("createPageAnnotation", [n2, e, t, r]);
  }
  updatePageAnnotation(n2, e, t, r) {
    return this.send("updatePageAnnotation", [n2, e, t, r]);
  }
  removePageAnnotation(n2, e, t) {
    return this.send("removePageAnnotation", [n2, e, t]);
  }
  getPageTextRects(n2, e) {
    return this.send("getPageTextRects", [n2, e]);
  }
  searchInPage(n2, e, t, r) {
    return this.send("searchInPage", [n2, e, t, r]);
  }
  getAnnotationsBatch(n2, e) {
    return this.send("getAnnotationsBatch", [n2, e]);
  }
  searchBatch(n2, e, t, r) {
    return this.send("searchBatch", [n2, e, t, r]);
  }
  getAttachments(n2) {
    return this.send("getAttachments", [n2]);
  }
  addAttachment(n2, e) {
    return this.send("addAttachment", [n2, e]);
  }
  removeAttachment(n2, e) {
    return this.send("removeAttachment", [n2, e]);
  }
  readAttachmentContent(n2, e) {
    return this.send("readAttachmentContent", [n2, e]);
  }
  getDocumentJavaScriptActions(n2) {
    return this.send("getDocumentJavaScriptActions", [n2]);
  }
  getPageAnnoWidgets(n2, e) {
    return this.send("getPageAnnoWidgets", [n2, e]);
  }
  getPageWidgetJavaScriptActions(n2, e) {
    return this.send("getPageWidgetJavaScriptActions", [n2, e]);
  }
  setFormFieldValue(n2, e, t, r) {
    return this.send("setFormFieldValue", [n2, e, t, r]);
  }
  setFormFieldState(n2, e, t, r) {
    return this.send("setFormFieldState", [n2, e, t, r]);
  }
  renameWidgetField(n2, e, t, r) {
    return this.send("renameWidgetField", [n2, e, t, r]);
  }
  shareWidgetField(n2, e, t, r, o) {
    return this.send("shareWidgetField", [n2, e, t, r, o]);
  }
  regenerateWidgetAppearances(n2, e, t) {
    return this.send("regenerateWidgetAppearances", [n2, e, t]);
  }
  flattenPage(n2, e, t) {
    return this.send("flattenPage", [n2, e, t]);
  }
  extractPages(n2, e) {
    return this.send("extractPages", [n2, e]);
  }
  createDocument(n2) {
    return this.send("createDocument", [n2]);
  }
  importPages(n2, e, t, r) {
    return this.send("importPages", [n2, e, t, r]);
  }
  deletePage(n2, e) {
    return this.send("deletePage", [n2, e]);
  }
  extractText(n2, e) {
    return this.send("extractText", [n2, e]);
  }
  redactTextInRects(n2, e, t, r) {
    return this.send("redactTextInRects", [n2, e, t, r]);
  }
  applyRedaction(n2, e, t) {
    return this.send("applyRedaction", [n2, e, t]);
  }
  applyAllRedactions(n2, e) {
    return this.send("applyAllRedactions", [n2, e]);
  }
  flattenAnnotation(n2, e, t) {
    return this.send("flattenAnnotation", [n2, e, t]);
  }
  exportAnnotationAppearanceAsPdf(n2, e, t) {
    return this.send("exportAnnotationAppearanceAsPdf", [n2, e, t]);
  }
  exportAnnotationsAppearanceAsPdf(n2, e, t) {
    return this.send("exportAnnotationsAppearanceAsPdf", [n2, e, t]);
  }
  getTextSlices(n2, e) {
    return this.send("getTextSlices", [n2, e]);
  }
  getPageGlyphs(n2, e) {
    return this.send("getPageGlyphs", [n2, e]);
  }
  getPageGeometry(n2, e) {
    return this.send("getPageGeometry", [n2, e]);
  }
  getPageTextRuns(n2, e) {
    return this.send("getPageTextRuns", [n2, e]);
  }
  merge(n2) {
    return this.send("merge", [n2]);
  }
  mergePages(n2) {
    return this.send("mergePages", [n2]);
  }
  preparePrintDocument(n2, e) {
    return this.send("preparePrintDocument", [n2, e]);
  }
  saveAsCopy(n2) {
    return this.send("saveAsCopy", [n2]);
  }
  closeDocument(n2) {
    return this.send("closeDocument", [n2]);
  }
  closeAllDocuments() {
    return this.send("closeAllDocuments", []);
  }
  setDocumentEncryption(n2, e, t, r) {
    return this.send("setDocumentEncryption", [n2, e, t, r]);
  }
  removeEncryption(n2) {
    return this.send("removeEncryption", [n2]);
  }
  unlockOwnerPermissions(n2, e) {
    return this.send("unlockOwnerPermissions", [n2, e]);
  }
  isEncrypted(n2) {
    return this.send("isEncrypted", [n2]);
  }
  isOwnerUnlocked(n2) {
    return this.send("isOwnerUnlocked", [n2]);
  }
};
d.READY_TASK_ID = "0";
var l = d;
var u = "ImageEncoderPool";
var c = "Encoder";
var p = class {
  constructor(n2 = 2, e, t) {
    this.poolSize = n2, this.workerUrl = e, this.workers = [], this.pendingTasks = /* @__PURE__ */ new Map(), this.nextWorkerId = 0, this.requestCounter = 0, this.logger = t ?? new ft(), this.initialize();
  }
  initialize() {
    this.logger.debug(u, c, `Creating worker pool with ${this.poolSize} workers`);
    for (let n2 = 0; n2 < this.poolSize; n2++) try {
      const e = new Worker(this.workerUrl, { type: "module" });
      e.onmessage = this.handleWorkerMessage.bind(this), e.onerror = this.handleWorkerError.bind(this), this.workers.push(e), this.logger.debug(u, c, `Worker ${n2} created successfully`);
    } catch (e) {
      this.logger.error(u, c, `Failed to create worker ${n2}:`, e);
    }
  }
  handleWorkerMessage(n2) {
    const e = n2.data, t = this.pendingTasks.get(e.id);
    if (t) if (this.pendingTasks.delete(e.id), "result" === e.type) t.resolve(e.data);
    else {
      const n3 = e.data;
      t.reject(new Error(n3.message));
    }
    else this.logger.warn(u, c, `Received response for unknown task: ${e.id}`);
  }
  handleWorkerError(n2) {
    this.logger.error(u, c, "Worker error:", n2.message);
  }
  getNextWorker() {
    if (0 === this.workers.length) return null;
    const n2 = this.workers[this.nextWorkerId];
    return this.nextWorkerId = (this.nextWorkerId + 1) % this.workers.length, n2;
  }
  encode(n2, e = "image/png", t) {
    return new Promise((r, o) => {
      const a = this.getNextWorker();
      if (!a) return void o(new Error("No workers available in the pool"));
      const i2 = `encode-${Date.now()}-${this.requestCounter++}`;
      this.pendingTasks.set(i2, { resolve: r, reject: o });
      const s2 = { id: i2, type: "encode", data: { imageData: { data: n2.data, width: n2.width, height: n2.height }, imageType: e, quality: t } };
      this.logger.debug(u, c, `Sending encoding request ${i2} (${n2.width}x${n2.height})`), a.postMessage(s2, [n2.data.buffer]);
    });
  }
  destroy() {
    this.logger.debug(u, c, "Destroying worker pool"), this.pendingTasks.forEach((n2, e) => {
      n2.reject(new Error("Worker pool destroyed")), this.logger.debug(u, c, `Rejected pending task: ${e}`);
    }), this.pendingTasks.clear(), this.workers.forEach((n2, e) => {
      n2.terminate(), this.logger.debug(u, c, `Worker ${e} terminated`);
    }), this.workers = [];
  }
  get activeWorkers() {
...
`], { type: "application/javascript" })), m2 = new p(i2 ?? 2, c2, a);
  return new h(u2, { imageConverter: x(m2), logger: a });
}
export {
  m as createPdfiumEngine
};
//# sourceMappingURL=worker-engine-m0rlHn8R-GL3JSWUY.js.map
