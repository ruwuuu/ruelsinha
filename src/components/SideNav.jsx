...
const navLinks = [
  ...
  { to: '/academics', icon: GraduationCap, text: 'Academics' },
  ...
  return (
    <motion.li variants={itemVariants} className="w-full">
      <Link
        to={link.to}
        onClick={onNavClick}
        className={`flex flex-row items-center justify-start gap-4 text-lg font-medium transition-colors duration-200 py-3 w-full
          ${isActive ? 'text-primary' : 'text-foreground hover:text-primary'}
        `}
        style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}
      >
        <Icon className="w-5 h-5 shrink-0" style={{ display: 'inline-block' }} />
        <span className="leading-none" style={{ display: 'inline-block' }}>
          {link.text}
        </span>
        <ArrowUpRight className="w-4 h-4 ml-auto shrink-0 opacity-50" />
      </Link>
    </motion.li>
  );
});
...
const SideNav = memo(({ open, onClose }) => {
  ...
  const navItems = useMemo(
    () =>
      navLinks.map((link) => (
        <NavItem key={link.to} link={link} onNavClick={onClose} isActive={location.pathname === link.to} />
      )),
    ...
    <>
      ...
      >
        ...
        <motion.button
          ...
          className="self-end text-muted-foreground hover:text-primary transition-all duration-300 hover:rotate-90 p-2 -mr-2 mb-8 will-change-transform cursor-pointer"
